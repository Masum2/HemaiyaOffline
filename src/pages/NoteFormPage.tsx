...
export const NoteFormPage: React.FC = () => {
  ...
  const handleCaseSelect = (caseName: string) => {
    ...
    const selectedCase = getCaseById(caseName);
    if (selectedCase) {
      // Set client type based on selected case
      if (selectedCase.isGroup) {
        updateField('clientType', '3'); // Group
        updateField('parentNames', selectedCase.parents || []);
        updateField('childNames', selectedCase.children || []);
        ...
        if (selectedCase.parents && selectedCase.parents.length > 0) {
          ...
          updateField('clientName', selectedCase.parents.join(', '));
          updateField('clientId', selectedCase.parents[0] || '');
        } else if (selectedCase.children && selectedCase.children.length > 0) {
          updateField('clientType', '0'); // Child
          updateField('childNames', selectedCase.children);
          updateField('parentNames', []);
          updateField('clientName', selectedCase.children.join(', '));
          updateField('clientId', selectedCase.children[0] || '');
        }
      }
    }
  };
      ...
      caseNoteService.getNoteById(Number(id)).then((note) => {
        if (note) {
          ...
          if (isOnline && rest.caseName) {
            ...
            if (caseData) {
              if (caseData.isGroup) {
                updateField('parentNames', caseData.parents || []);
                updateField('childNames', caseData.children || []);
              } else {
  ...
  const renderClientFields = () => {
    ...
    if (isOnline) {
      if (isGroup) {
        ...
        let parentOptions: string[] = getAllParents() || [];
        let childOptions: string[] = getAllChildren() || [];
        ...
        if (formData.selectedCase) {
          ...
          if (caseParents.length > 0) parentOptions = caseParents;
          if (caseChildren.length > 0) childOptions = caseChildren;
        }
